...
uses
    ...
    SimUnit in 'SimUnit.pas'; (* Library containing useful functions/procedures *)
      ...
      BEGIN
         if (NbOper > 0) then // SHOULD ALWAYS BE TRUE!
           ...
           if (NbOper = 2) and ( not(FormatF1(OpName)) ) then
             ...
             writeln( ' at address $', Word2Hex(PC-2) );
             H := true;
             END
           END
      END;
...
(* (TMPS, TMPD, & TMPR) and the memory or the Dn and An registers it would be *)
...
(******************************************************************************)
 ...
 //// Private to Controller
   function CheckCond(Cond: boolean; msg: string):boolean;
...
(*     iDIVS: BEGIN
     ...
     iDIVS: BEGIN
		...
		if CheckCond( (TMPS <> $0), 'Division by Zero' ) then
                  ...
                  remflip := false ;
                  FillTMP(TMPD, OpAddr2, Size, M2, N2);
                  if TMPD > $7FFFFFFF then {TMPD is negative}
                    BEGIN
                    TMPD := (TMPD xor $FFFFFFFF) + $1 ; {get 2's C}
                    rev := true ;       {will have to flip the sign of result}
                    remflip := true ;   {will have to flip the sign of remainder}
                    END ;
                  if TMPS > $7FFF then {TMPS is negative}
                    BEGIN
                    TMPS := (TMPS xor $FFFF) + $1 ;  {get 2's C}
                    rev := rev xor true ; {flip the state of rev}
                    END ;
                  TMPR := TMPD div GetWord(TMPS, false) ;

                  {set the overflow bit if there is any value in the high word}
                  V := ( GetWord(TMPR, true) > $0 );

                  {flip the sign of result if necessary}
                  if rev then TMPR := (TMPR xor $FFFFFFFF) + $1 ;

                  {get the remainder}
                  tmpA := TMPD mod GetWord(TMPS, false) ;
                  {flip the sign of remainder if necessary}
                  if remflip then tmpA := (tmpA xor $FFFF) + $1 ;
                  {set remainder to the high word of TMPR; DIV is the low word}
                  SetWord(TMPR, true, tmpA) ;
     ...
     iROL: BEGIN
            FillTMP(TMPD, OpAddr2, Size, M2, N2);
            {MUST BRACKET '8*NOB(Size)' TO GET PROPER PRECEDENCE!}
            Data := Data mod (8*NOB(Size));
            TMPR := TMPD shl Data;
            {MUST BRACKET '8*NOB(Size)-Data' TO GET PROPER PRECEDENCE!}
